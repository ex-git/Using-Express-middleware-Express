...
function gateKeeper(req, res, next) {
  ...
  const usernamePassword = req.get('x-username-and-password');
  const parsedUsernamePassword = queryString.parse(usernamePassword);
  req.user = USERS.find(user=> user.userName === parsedUsernamePassword.user && user.password === parsedUsernamePassword.pass);
  next();
}
